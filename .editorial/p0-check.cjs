...
for (const [file, approval] of Object.entries(review.files)) {
    ...
    assert.equal(hash(read(file)), approval.after, `Unreviewed P0 page change: ${file}`);
}
...
for (const file of protectedFiles) assert.equal(hash(read(file)), hash(baseline(file)), `Protected system: ${file}`);
...
module.exports = { review, previous, baseline };
